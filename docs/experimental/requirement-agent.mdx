...
```py
# Create a requirement agent with conditional tool execution
...
```py
# Force execution order: ThinkTool first, then DuckDuckGoSearchTool
RequirementAgent(
  llm=ChatModel.from_name("ollama:granite3.3:8b"),
...
```py
# Create a ReAct agent that forces thinking after tool usage
RequirementAgent(
  llm=ChatModel.from_name("ollama:granite3.3:8b"),
...
```py
# ReAct agent with custom conditions and priorities
RequirementAgent(
  llm=ChatModel.from_name("ollama:granite3.3:8b"),
...
```py
# Prevent early termination before tool is called
...
```py
# Complete parameter reference for ConditionalRequirement
...
```py
# Agent that asks for permission before using specific tools
RequirementAgent(
  llm=ChatModel.from_name("ollama:granite3.3:8b"),
...
```py
# Complete parameter reference for AskPermissionRequirement
